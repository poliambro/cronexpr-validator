...
class Validator:
    ...
    def validate_seconds_and_minutes(seconds_and_minutes_expression: str) -> bool:
        ...
        """
        is_range_valid = Validator.__validate_range(seconds_and_minutes_expression,
                                                    constants.SECONDS_AND_MINUTES_RANGE_REGEX_EXPRESSION)
    ...
    def validate_hours(hours_expression: str) -> bool:
        ...
        """
        is_range_valid = Validator.__validate_range(hours_expression, constants.HOURS_RANGE_REGEX_EXPRESSION)
        if not is_range_valid:
    ...
    def validate_day_of_month(day_of_month_expression: str) -> bool:
        ...
        """
        is_range_valid = Validator.__validate_range(day_of_month_expression,
                                                    constants.DAY_OF_MONTH_RANGE_REGEX_EXPRESSION)
        if not is_range_valid:
    ...
    def validate_month(month_expression: str) -> bool:
        ...
        if any(alternative_value in month_expression for alternative_value in constants.MONTHS_LIST):
            is_range_valid = Validator.__validate_range(month_expression,
                                                        constants.MONTH_ALTERNATIVE_VALUES_RANGE_REGEX_EXPRESSION,
                                                        constants.MONTHS_LIST)
            if not is_range_valid:
                return len(re.findall(constants.MONTH_ALTERNATIVE_VALUES_REGEX_EXPRESSION, month_expression)) > 0
            return is_range_valid
        is_range_valid = Validator.__validate_range(month_expression, constants.MONTH_NUMBERS_RANGE_REGEX_EXPRESSION)
        if not is_range_valid:
    ...
    def validate_day_of_week(day_of_week_expression: str) -> bool:
        ...
        if any(alternative_value in day_of_week_expression for alternative_value in constants.DAY_OF_WEEK_LIST):
            is_range_valid = Validator.__validate_range(day_of_week_expression,
                                                        constants.DAY_OF_WEEK_ALTERNATIVE_VALUES_RANGE_REGEX_EXPRESSION,
                                                        constants.DAY_OF_WEEK_LIST)
            if not is_range_valid:
                return len(re.findall(constants.DAY_OF_WEEK_ALTERNATIVE_VALUES_REGEX_EXPRESSION,
                                      day_of_week_expression)) > 0
            return is_range_valid
        is_range_valid = Validator.__validate_range(day_of_week_expression,
                                                    constants.DAY_OF_WEEK_NUMBERS_RANGE_REGEX_EXPRESSION)
        if not is_range_valid:
    ...
    def validate_year(year_expression: str) -> bool:
        ...
        """
        is_range_valid = Validator.__validate_range(year_expression, constants.YEAR_RANGE_REGEX_EXPRESSION)
        if not is_range_valid:
            return len(re.findall(constants.YEAR_REGEX_EXPRESSION, year_expression)) > 0
        return is_range_valid

    @staticmethod
    def __validate_range(range_expression: str, regex_expression: str, range_list: list = []) -> bool:
        range_match = re.findall(regex_expression, range_expression)
